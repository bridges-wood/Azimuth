...
public class Terminal extends Object implements Serializable {
	...
	public void hack(int difficulty, int hackingSkill) {
		int dim = (int) Math.ceil((double) (1 / difficulty
				* 10)); /*
						 * Calculates the dimensions of the array as ten times the reciprocal of the
						 * difficulty. This is then rounded up to avoid 0 length arrays. TODO check this
						 * and make sure it doesn't break also check balance
						 */
		...
		for (int x = 0; x < dim; x++) {
			for (int y = 0; y < dim; y++) {
				try {
					passwordsPoss[x][y] = Utilities.generatePassword(difficulty); // Populates the array with random
																					// passwords.
		...
		System.out.println(Utilities.rangedRandomInt(0, 10));
